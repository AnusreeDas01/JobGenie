...
class PDF(FPDF):
    def __init__(self):
        ...
        # ✅ Add DejaVu font once when PDF object is created
        font_path = os.path.join("core", "DejaVuSans.ttf")
    ...
    def header(self):
        ...
        self.cell(0, 10, "✨ JobGenie Resume Feedback Report ✨", 0, 1, "C")
...
def export_pdf_with_style(score, matched, total, matched_keywords, missing_keywords, summary, skill_gap, suggestions, rewritten):
    ...
    pdf.section("📊 Match Score", f"{score}% ({matched}/{total} skills matched)")
    pdf.section("✅ Matched Skills", matched_keywords, bullet=True)
    pdf.section("❌ Missing Skills", missing_keywords, bullet=True)
    pdf.section("📝 Career Summary", summary)
    pdf.section("🔍 Skill Gaps", skill_gap, bullet=True)
    pdf.section("💡 Resume Suggestions", suggestions, bullet=True)
    pdf.section("✨ Rewritten Resume Bullet", rewritten)

    pdf.set_y(-20)
    pdf.set_font("DejaVu", "", 12)
    pdf.set_text_color(120, 120, 120)
    pdf.cell(0, 10, f"Generated by JobGenie | {datetime.now().strftime('%d %b %Y')}", 0, 0, "C")
    pdf.multi_cell(0, 10, f"Match Score: {score}% ({matched}/{total})")

    filename = "JobGenie Report.pdf"
    pdf.output(filename)
